...
只要上游素材流程需要生成、扩图、修复或重绘静态 PNG，必须先读 [../../marketplace/pptx/editable-reconstruction.md](../../marketplace/pptx/editable-reconstruction.md)，复用其中的图片模型优先级、无字生成和残影验收规则。生成图片中不得烘焙标题、数字、标签、字幕或伪文字；这些内容必须由 manifest、`video.config.json` 和 HyperFrames/HTML 层承载。

本 skill 仍只消费通过 manifest 校验的真实 PNG。该引用不允许绕过 `it-infra-continuous-png` 前置条件，也不允许用 CSS、SVG 或伪 PNG 代替输入。
