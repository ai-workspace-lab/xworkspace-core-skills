...
只要任务需要生成、扩图、修复或重绘静态图片，必须先读 [../../marketplace/pptx/editable-reconstruction.md](../../marketplace/pptx/editable-reconstruction.md)，复用其中的图片模型优先级、无字生成和残影验收规则。生成图片中不得烘焙产品名、标题、数字、标签、CTA、字幕或伪文字；这些内容必须留在 HyperFrames/HTML 可编辑动画层。

官网截图、真实产品界面、Logo 和用户素材仍然优先。生成图只能补足背景、氛围或解释性插画，不能伪装成真实产品截图或品牌证据。
