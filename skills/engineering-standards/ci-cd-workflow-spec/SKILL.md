...
- **A skip propagates down the whole `needs` chain, and `always()` only stops it for the job that carries it.** If an early job is skipped and the next one runs *only* because its condition starts with `always()`, that job's success does not clear the skip for anything downstream: the next job's `if` is never evaluated at all. The symptom is a job whose condition is correct in every operand yet reports `skipped` forever — so the operands get audited over and over while the actual defect is a missing `always()`. **Every job downstream of an `always()` job needs its own `always()`**, paired with an explicit `needs.<upstream>.result == 'success'` — catching the skip must not also make the job run after a real upstream failure.
  - **Diagnostic:** compare a step-level and a job-level read of the same output. If a step gated on `steps.<id>.outputs.X == 'true'` ran, but a downstream job gated on `needs.<job>.outputs.X == 'true'` skipped, the value is fine and the propagation is not. Equivalently, if exactly the jobs carrying `always()` ran and exactly those without it skipped, the conditions are not what is being tested.
