...
- 运行时环境变量仅可承载 OIDC 登录参数或本次 Job 从 Vault 读取后的短期值。变量优先级为：运行时环境变量 → Vault 查询 → 安全的非敏感默认值（如 `127.0.0.1`）。凭证、目标主机、环境路径和依赖服务端点等必填值缺失时 MUST 通过 `assert` 或 `fail` 明确失败；绝对禁止在 Playbook、Ansible `defaults/main.yml`、模板或角色中硬编码生产域名、IP 或生产数据库 DSN 作为退回 fallback。模板中的依赖服务 URL 必须根据当前环境/主机的核心网络域名配置动态派生。
