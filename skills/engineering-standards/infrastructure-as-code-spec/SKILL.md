...
- **声明归属**: 多环境拓扑声明只放在 GitOps 仓库的 `resources/<project>/<env>/<provider>/*.yaml`；IaC 仓库只保留可复用的 Terraform 模块、模板、渲染器和 workspace/backend 代码。`<project>`、`<env>`、`<provider>` 三元组是声明的唯一归属，不得在多个仓库复制或双写。
...
- **消费者契约**: 渲染器必须通过必填的 `--resources`（或等价 `RESOURCES`）参数接收声明目录，路径解析失败或目录为空时立即失败；不得把声明路径写死在模块中，也不得为兼容旧路径而复制一份拓扑。
...
- **拓扑声明与模块的归属判据**: 模块（HCL + 模板 + 渲染器）是可复用代码，GitOps 拓扑声明（`resources/<project>/<env>/<provider>/**`）是随环境变动的数据，两者变更频率不同，不应被同一个 ref 绑在一起。判断能否分离，看三点：声明里是否引用了模块内部标识（`source =`、模块路径）、渲染器是否把声明路径写死（还是作为 `--resources` 之类的参数传入）、生成产物目录是否签入仓库。三者都为"否"时耦合仅剩数据，可以外移；此时**优先移入唯一消费者仓库**，而不是新开一个仓 —— 独立成仓只在出现第二个消费者、或声明需要不同访问控制（含 SSH 公钥、完整主机拓扑）时才划算。
