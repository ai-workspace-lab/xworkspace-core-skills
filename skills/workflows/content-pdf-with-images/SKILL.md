...
Before generating, extending, repairing, or repainting chapter images, read [../../marketplace/pdf/editable-reconstruction.md](../../marketplace/pdf/editable-reconstruction.md) and apply its provider order, text-free generation, and residue QA rules. Generated chapter images must contain no titles, labels, numbers, watermarks, or pseudo-text. Keep all article text and captions in the PDF layout layer.

When the source is a scanned, flattened, or image-only PDF that must be visually reconstructed, apply the complete per-page contract from that reference: one text-free full-page PNG plus native selectable PDF text objects.
