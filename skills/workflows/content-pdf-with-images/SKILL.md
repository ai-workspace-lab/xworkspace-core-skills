...
Before generating, extending, repairing, or repainting chapter images, read [../../marketplace/pdf/editable-reconstruction.md](../../marketplace/pdf/editable-reconstruction.md) only for its provider order. Generate the chapter image that best fits the intended composition; do not force a text-free background or separate PDF text layer for ordinary illustrations.

Only when the user explicitly requests editable reconstruction of a scanned, flattened, or image-only PDF, apply the complete per-page contract from that reference: one text-free full-page PNG plus native selectable PDF text objects.
